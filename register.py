...
class RegisterWindow(QWidget):
    ...
    def add_operator(self):
        ...
        try:
            ...
            self.db.cur.execute(
                """
                INSERT INTO postgres.session.operator 
                (full_name, gender_id, phone, birth_date, login, parol) 
                VALUES (%s, %s, %s, %s, %s, %s)
                """,
                (fio, gender_id, phone, date, login, password)
            )
            self.db.commit()
